...
shinyServer(function(input, output) {
	...
	chosenColor <- reactive({
		input$color
	})
	chosenMapType <- reactive({
		input$type
	})
	chosenState <- reactive({
		input$state
	})
	numberOfCategories <- reactive({
		input$ncats
	})
	...
	# This will allow the user to choose custom labels.
	usingCustomLabels <- reactive({
		input$usecustomcategories
	})
	output$customcategories <- renderUI({
		n <- numberOfCategories()
		temp <- list(n+1)
		temp[[1]] <- helpText("Smallest to largest.")
		for ( i in 1:n) {
...
>>>>>>> Updated to be compatible with latest Shiny
